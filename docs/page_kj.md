...
窗体是一个容器，拖出到页面后，双击进入窗体内部，可以放置其它组件，组成一个应用对话框。

容器操作，参考 “布局” 章节。

运行时，窗体默认为隐藏状态，由窗体组件实例的输入数据点触发显示。

<p class="tip">说明，窗体内部的组件，在发布后同样在后面板生成组件实例</p>
...
发布后，在后面板生成复选框组件实例，有2个数据点：

![编辑器](../image/m5.png)

**输出**数据点： 选中时输出1，取消选中输出0

**输入**数据点： 用于初始化复选框，输入1显示选中状态，输入0显示取消选中状态
